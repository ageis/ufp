...
class SummaryFormatter(BaseFormatter):
    ...
    def format(self):
        ...
        print('Source ports: ' +
              ', '.join([str(value)
                         for value in sorted(self.src_ports)]) + '\n\n')
        print('Destination ports: ' +
              ', '.join([str(value)
                         for value in sorted(self.dst_ports)]) + '\n\n')
        if self.options.reverse_dns:
            print('Source Hostnames: ' + ', '.join([
                '[%s] %s' % (key, value)
                for (key, value) in self.src_hosts.items()
            ]) + '\n\n')
            print('Destination Hostnames: ' + ', '.join([
                '[%s] %s' % (key, value)
                for (key, value) in self.dst_hosts.items()
            ]) + '\n\n')
...
class SrcToDstPortFormatter(BaseFormatter):
    ...
    def format(self):
        ...
        for src in sorted(self.dst_ports_by_src_ip.keys()):
            print("%s\t%s" % (src, ', '.join([
                str(value) for value in sorted(self.dst_ports_by_src_ip[src])
            ])))
...
class DstToDstPortFormatter(BaseFormatter):
    ...
    def format(self):
        ...
        for dst in sorted(self.dst_ports_by_dst_ip.keys()):
            if self.options.reverse_dns:
                print("{dsthost:75} [{dstip:39}] {dstports}".format(
                    dsthost=self.hosts[dst],
                    dstip=dst,
                    dstports=', '.join(self.dst_ports_by_dst_ip[dst])))
            else:
                print("%s\t\t%s" % (dst, ', '.join([
                    str(value)
                    for value in sorted(self.dst_ports_by_dst_ip[dst])
                ])))
...
class SrcToDstIPFormatter(BaseFormatter):
    ...
    def format(self):
        ...
        for dst in sorted(self.src_ips_by_dst_ip.keys()):
            if self.options.reverse_dns:
                print("%s (%s)\t\t%s" % (dst, self.hosts[dst], ', '.join([
                    str(value) for value in sorted(self.src_ips_by_dst_ip[dst])
                ])))
            else:
                print("%s\t%s" % (dst, ', '.join([
                    str(value) for value in sorted(self.src_ips_by_dst_ip[dst])
